...
class BlockMiddlewaretwo:
    """
    Middleware to block specific IPs and user agents.
    """
    ...
    def __call__(self, request):
        """
        Process request and block IPs and user agents if necessary.
        """
        # List of blocked IPs
        blocked_ips = [
        ...
        # Check if client IP is in blocked IPs list
        for pattern in blocked_ips:
            if re.match(pattern, client_ip):
                self.capture_ip(client_ip)
                return HttpResponse("<h1>404 Not Found</h1>The page that you have requested could not be found.")

        # Check if user agent is in blocked user agents list
        for agent in user_agents:
            if agent in user_agent or not user_agent.strip():
                self.capture_ip(client_ip)
                return HttpResponse("<h1>404 Not Found</h1>The page that you have requested could not be found.")

        # Continue with the request if no blocking condition is met
        response = self.get_response(request)
        return response

    def capture_ip(self, ip):
        """
        Log captured IP to a file.
        """
        with open(settings.BASE_DIR / 'captured.txt', 'a+') as f:
            f.write(f"ip : {ip} | {timezone.now().strftime('%d/%m/%Y %I:%M:%S%p')}\n\n")
